...
export default function GeoPage() {
  ...
  return (
    <PageGate page="geo"><div className="space-y-6">
      ...
      {/* France Map — full width, has its own internal layout */}
      <FranceMap />

      {/* KPI summary cards — 3-column grid below the map */}
      {!loading && filteredStoreGroups.length > 0 && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {/* Points de vente */}
          <SectionCard title="Points de vente" icon={Store} iconColor="bg-violet-100 text-violet-600">
          ...
          {!gmbLoading && gmbScoring && filteredGmbCompetitors.length > 0 && (
            <SectionCard title="Google My Business" icon={Star} iconColor="bg-amber-100 text-amber-600">
              <div className="grid grid-cols-2 gap-3">
                <div>
          ...
          {/* Repartition */}
          <SectionCard title="Repartition" icon={BarChart3} iconColor="bg-violet-100 text-violet-600">
            <div className="space-y-1.5">
              {filteredStoreGroups.slice(0, 6).map((g) => {
                ...
                return (
                  <div key={g.competitor_id}>
                    ...
                    <div className="h-1.5 bg-muted rounded-full overflow-hidden">
                      <div
                        className={`h-full rounded-full ${isBrand ? "bg-gradient-to-r from-violet-500 to-violet-400" : "bg-gray-400"}`}
      ...
      {!gmbLoading && gmbScoring && filteredGmbCompetitors.length > 0 && (
        <div className="space-y-4">
          <SectionCard title="Scoring Google My Business — Detail" icon={Star} iconColor="bg-amber-100 text-amber-600" noPadding>
                  ...
                  {filteredGmbCompetitors.map((comp) => {
                    ...
                    return (
                      <tr
                        ...
                        className={`border-b border-border last:border-0 hover:bg-muted/50 transition-colors ${isBrand ? "bg-violet-50/40" : ""}`}
                      >
                        ...
                        <td className="py-2.5 px-4">
                          <div className="flex items-center gap-2">
                            <div className="w-20 h-2 bg-muted rounded-full overflow-hidden">
                              <div
                                className={`h-full rounded-full ${isBrand ? "bg-gradient-to-r from-violet-500 to-violet-400" : getScoreColor(comp.avg_score)}`}
          ...
          {filteredGmbCompetitors.some(c => c.top_stores.length > 0) && (
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              {/* Top 5 */}
              <SectionCard title="Top magasins" icon={Trophy} iconColor="bg-emerald-100 text-emerald-600" className="border-emerald-200">
              ...
              {/* Flop 5 */}
              <SectionCard title="Magasins a ameliorer" icon={ThumbsDown} iconColor="bg-red-100 text-red-600" className="border-red-200">
                <div className="space-y-2">
                  {filteredGmbCompetitors
      ...
      {!loading && filteredStoreGroups.length > 0 && (
        <div className="space-y-4">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            ...
            <SectionCard title="Repartition des points de vente" icon={Store} iconColor="bg-violet-100 text-violet-600">
              <div className="space-y-2">
                {filteredStoreGroups.map((g, i) => {
                  ...
                  const barColor = isBrand ? "bg-gradient-to-r from-violet-500 to-violet-400" : (barColors[rc.text] || "bg-gray-300");
                  const pct = totalStores > 0 ? (g.total / totalStores * 100) : 0;
                  return (
                    <div key={g.competitor_id}>
            ...
            {catchmentData && catchmentData.competitors?.length > 0 && (
              <SectionCard
                ...
                action={<span className="text-[11px] font-normal text-muted-foreground">Rayon {catchmentData.radius_km} km</span>}
              >
                <div className="space-y-2">
                  ...
                  <div className="mt-3 pt-3 border-t border-border">
                    ...
                    <div className="space-y-1.5">
                      {catchmentData.overlaps.slice(0, 4).map((o: any, i: number) => (
                        <div key={i} className="rounded-lg bg-muted/50 px-2.5 py-1.5 flex items-center justify-between">
                          <span className="text-[11px] text-muted-foreground truncate">{o.competitor_a_name} / {o.competitor_b_name}</span>
                          <span className="text-[11px] font-semibold text-violet-700 ml-2 whitespace-nowrap">
          ...
          {recommendations.length > 0 && (
            <SectionCard title="Insights geographiques" icon={Sparkles} iconColor="bg-violet-100 text-violet-600" className="border-violet-200 bg-gradient-to-br from-violet-50/50 to-violet-100/30">
              <div className="space-y-3">
                {recommendations.map((rec, i) => (
                  <div key={i} className="rounded-xl bg-card/80 border border-violet-100 p-4 text-sm text-violet-900">
                    {rec}
                  </div>
                ))}
              </div>
